...
/*
 服务器请求操作处理
 */
@implementation wechat_qrcodePayHandler
...
-(BOOL) init:(NSString *)app_id mch_id:(NSString *)mch_id dic:(NSDictionary *)dic
{
    ...
    [_infoDic addEntriesFromDictionary:dic];
...
- ( NSMutableDictionary *)sendPay_WithInfoDic:(NSDictionary *)infoDic
{
    ...
    CGFloat price = [[infoDic objectForKey:@"price"] floatValue] * 100;
    ...
    //    [packageParams setObject: @"APP-001"        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"JSAPI"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: [self.infoDic objectForKey:@"order_name"]        forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: [self.infoDic objectForKey:@"order_number"]         forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"127.0.0.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: priceStr       forKey:@"total_fee"];       //订单金额，单位为分
//    [packageParams setObject:@"12354323" forKey:@"product_id"];
